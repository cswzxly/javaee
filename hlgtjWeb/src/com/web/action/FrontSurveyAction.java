...
//			    int result = endDate.compareTo(nowDate);
		...
		if(index.equals("1")){ //正在调查
			if(listSurvey1 != null && listSurvey1.size() > 0){
				for(Survey survey : listSurvey1){
			...
			if(listSurvey2 != null && listSurvey2.size() > 0){
				for(Survey survey : listSurvey2){
					survey.setDesId(des.getEncString(survey.getTheID().toString()));
				}
			}
			request.setAttribute("list0", listSurvey2);
		}
		request.setAttribute("index", des.getEncString(index));
		return "success";
	}
