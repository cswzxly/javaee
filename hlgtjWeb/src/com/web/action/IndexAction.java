...
//		request.setAttribute("contents45List", contents45List);
        ...
        //政务动态头条
        logger.info(">>>>>>>>>>IndexAction:index:政务动态头条>>>>>>>>>>");
        Contents zwdtContents = new Contents();
        value = PropertiesUtils.readValue(propertiesPath, "zwdtCatalogID");
        zwdtContents.setCatalogID(Integer.valueOf(value));
        zwdtContents.setStatus("1");
        zwdtContents.setTopNum(null);
        zwdtContents.setIsTop("1");
        logger.info("---------------------------"+new Date());
        zwdtContents = frontContentsServices.queryContentsByBean(zwdtContents);
        logger.info("---------------------------"+new Date());
        //政务动态
        logger.info(">>>>>>>>>>IndexAction:index:政务动态>>>>>>>>>>");
        if (zwdtContents != null && zwdtContents.getTheID() != null) {//如果有头条新闻，则查询7条新闻
            zwdtContents.setContents(HtmlRegexpUtil.Html2Text(zwdtContents.getContents()));
            zwdtContents.setDesId(des.getEncString(zwdtContents.getTheID().toString()));
            request.setAttribute("zwdtContents", zwdtContents);
            zwdtContents = new Contents();
            zwdtContents.setCatalogID(Integer.valueOf(value));
            zwdtContents.setStatus("1");
            zwdtContents.setTopNum("6");
            zwdtContents.setIsTop("0");
            logger.info("-----  政务动态---------------"+new Date());
            List<Contents> zwdtContentsList = frontContentsServices.queryTopContentsList(zwdtContents);
            logger.info("-------  政务动态------------"+new Date());
            Catalog zwdtCatalog = frontCatalogServices.queryCatalogByID(value);
            request.setAttribute("zwdtCatalog", zwdtCatalog);
            if (zwdtContentsList != null && zwdtContentsList.size() > 0) {
                for (Contents c : zwdtContentsList) {
                    c.setDesId(des.getEncString(c.getTheID().toString()));
                }
            }
            request.setAttribute("zwdtContentsList", zwdtContentsList);
        } else {//如果没有头条新闻，则查询8条新闻，第一条为头条新闻
            zwdtContents = new Contents();
            zwdtContents.setCatalogID(Integer.valueOf(value));
            zwdtContents.setStatus("1");
            zwdtContents.setTopNum("10");
            zwdtContents.setIsTop("0");
            List<Contents> zwdtContentsList = frontContentsServices.queryTopContentsList(zwdtContents);
            Catalog zwdtCatalog = frontCatalogServices.queryCatalogByID(value);
            request.setAttribute("zwdtCatalog", zwdtCatalog);
        ...
        value = PropertiesUtils.readValue(propertiesPath, "xwtpCatalogID");
        picBean.setCatalogID(Integer.valueOf(value));
        picBean.setIsindex("1");
        picBean.setFileType("1");
        picBean.setTopNum("4");
        logger.info("-----  新闻图片------start---------"+new Date());
        List<AllClassBean> picIndexList = frontFileInfoServices.queryTopPicIndex(picBean);
        logger.info("-----  新闻图片------end---------"+new Date());
...
//        request.setAttribute("content186List", content186List);
        ...
        //业务公告
        logger.info(">>>>>>>>>>IndexAction:index:业务公告>>>>>>>>>>");
        Contents ywggcontents = new Contents();
        Integer tdlyglCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "tdlyglCatalogID"));
        Integer kczyglCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "kczyglCatalogID"));
        Integer dzzhfzCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "dzzhfzCatalogID"));
        Integer chglCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "chglCatalogID"));
        Integer djCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "djCatalogID"));
        ids = new Integer[]{tdlyglCatalogID, kczyglCatalogID, dzzhfzCatalogID, chglCatalogID, djCatalogID};
        ywggcontents.setIds(ids);
        ywggcontents.setStatus("1");
        ywggcontents.setTopNum("8");
        logger.info("-----  业务公告------start---------"+new Date());
        List<Contents> ywggcontentsList = frontContentsServices.queryContentsByIds(ywggcontents);
        logger.info("-----  业务公告------end---------"+new Date());

        if (ywggcontentsList.size() > 0) {
            for (Contents c : ywggcontentsList) {
                c.setDesId(des.getEncString(c.getTheID().toString()));
            }
        }
        request.setAttribute("ywggcontentsList", ywggcontentsList);

        //政策法规
        logger.info(">>>>>>>>>>IndexAction:index:政策法规>>>>>>>>>>");
        Contents zcfgcontents = new Contents();
        Integer gjzcfgCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "gjzcfgCatalogID"));
        Integer szcfglCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "szcfglCatalogID"));
        Integer shizcfgCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "shizcfgCatalogID"));
        Integer zcjdCatalogID = Integer.valueOf(PropertiesUtils.readValue(propertiesPath, "zcjdCatalogID"));
        ids = new Integer[]{gjzcfgCatalogID, szcfglCatalogID, shizcfgCatalogID, zcjdCatalogID};
        zcfgcontents.setIds(ids);
        zcfgcontents.setStatus("1");
        zcfgcontents.setTopNum("8");
        logger.info("-----  政策法规------start---------"+new Date());
        List<Contents> zcfgcontentsList = frontContentsServices.queryContentsByIds(zcfgcontents);
        logger.info("-----  政策法规------end---------"+new Date());
        if (zcfgcontentsList.size() > 0) {
            for (Contents c : zcfgcontentsList) {
                c.setDesId(des.getEncString(c.getTheID().toString()));
            }
        }
        request.setAttribute("zcfgcontentsList", zcfgcontentsList);


        //人事信息
        logger.info(">>>>>>>>>>IndexAction:index:人事信息>>>>>>>>>>");
        Contents rsxxcontents = new Contents();
        value = PropertiesUtils.readValue(propertiesPath, "rsxxCatalogID");
        rsxxcontents.setCatalogID(Integer.valueOf(value));
        rsxxcontents.setStatus("1");
        rsxxcontents.setTopNum("8");
        logger.info("-----  人事信息------start---------"+new Date());
        List<Contents> rsxxcontentList = frontContentsServices.queryTopContentsList(rsxxcontents);
        logger.info("-----  人事信息------end---------"+new Date());
        if (rsxxcontentList.size() > 0) {
            for (Contents c : rsxxcontentList) {
                c.setDesId(des.getEncString(c.getTheID().toString()));
            }
        }
        request.setAttribute("rsxxcontentList", rsxxcontentList);

        //通知公告
        logger.info(">>>>>>>>>>IndexAction:index:通知公告>>>>>>>>>>");
        Contents tzggcontents = new Contents();
        value = PropertiesUtils.readValue(propertiesPath, "tzggCatalogID");
        tzggcontents.setCatalogID(Integer.valueOf(value));
        tzggcontents.setStatus("1");
        tzggcontents.setTopNum("8");
        logger.info("-----  通知公告------start---------"+new Date());
        List<Contents> tzggcontentsList = frontContentsServices.queryTopContentsList(tzggcontents);
        logger.info("-----  通知公告------end---------"+new Date());
        if (tzggcontentsList.size() > 0) {
            for (Contents c : tzggcontentsList) {
                c.setDesId(des.getEncString(c.getTheID().toString()));
            }
        }
        request.setAttribute("tzggcontentsList", tzggcontentsList);
...
//            }
//        }
//        request.setAttribute("content185List", content187List);


        //资料下载zlxzCatalogID=128
        logger.info(">>>>>>>>>>IndexAction:index:资料下载>>>>>>>>>>");
        FileInfo zlxz = new FileInfo();

        value = PropertiesUtils.readValue(propertiesPath, "zlxzCatalogID");
        zlxz.setCatalogID(Integer.valueOf(value));
        zlxz.setTopNum("8");
        List<FileInfo> fileList = frontFileInfoServices.front_fileInfoList(zlxz);
...
//        request.setAttribute("contents169List", contents169List);
    ...
    public String searchList() {
        ...
        s.setTitle(title);
        PageInfo<SearchVO> resultList = frontSearchServices.querySearchVOByBean(s, getPageNum(), getPageSize());
        request.setAttribute("size", resultList.getSize());
