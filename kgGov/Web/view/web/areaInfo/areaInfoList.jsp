                    ...
                    <ul>
                        <c:if test="${catalog.catalogType=='t_contents' }">
                            <c:forEach items="${contentsList.list}" var="item">
                              <li><a href="${ctx}/web/contentsShow.action?type=${leftTitle }&contentsID=${item.desId}">${item.title }</a><em>${fn:substring(item.releaseDate,0,10) }</em></li>
                              </c:forEach>
                        </c:if>
                        <c:if test="${catalog.catalogType=='t_video' }">
                            <c:forEach items="${contentsList.list}" var="item">
                              <li><a href="${ctx}/web/videoShow.action?type=${leftTitle }&videoID=${item.desId}">${item.title }</a><em>${fn:substring(item.releaseDate,0,10) }</em></li>
                           </c:forEach>
                        </c:if>
