...
<script>
    ...
    var contents_page = {
        ...
        initAccrualTypeWin:function(){
        	$('#accrualTypeWin').window({
                title: '权责清单',
                width: 300,
                modal: true,
                shadow: true,
        ...
        closeAccrualTypeWin:function(){
        	$('#accrualTypeWin').window('close');
        },

        deleteContents: function () {
            var catalogType = $("#catalogType").val();
            var url;
            if (catalogType == "t_contents" || catalogType == "t_oneContent") {
            ...
            } else if (catalogType == "t_accrual"){
            	url = "${ctx}/accrual/deleteAccrual.action";
            }else {
                url = "";
            }
            var param = {};
        ...
        $.post(url, param, function (data) {
            if (data.code == "00") {
                ...
                if (catalogType == 't_contents') {
                    ...
                    $('#dg').datagrid("hideColumn", "accrualTypeName");
...
//                     $('#fileInfoListBtn').hide();
                ...
                }else if (catalogType == 't_oneContent'){
                	$('#dg').datagrid("hideColumn", "linkTypeName");
                    $('#dg').datagrid("hideColumn", "selectTypeName");
                    $('#dg').datagrid("hideColumn", "link_url");
                    $('#dg').datagrid("hideColumn", "accrualTypeName");
//                    $('#dg').datagrid("hideColumn", "isindexName");
                    ...
                    $('#dg').datagrid("hideColumn", "isTopName");
                    $('#dg').datagrid("hideColumn", "isFocusName");
                }else if (data.catalogType == 't_accrual'){
                	$('#dg').datagrid("showColumn", "releaseDate");
                	$('#dg').datagrid("showColumn", "accrualTypeName");
                	$('#dg').datagrid("hideColumn", "source");
                    $('#dg').datagrid("hideColumn", "linkTypeName");
                    $('#dg').datagrid("hideColumn", "selectTypeName");
                    $('#dg').datagrid("hideColumn", "link_url");
        ...
        $('#addContentsBtn').click(function () {
            ...
            } else if ($("#catalogType").val() == 't_oneContent'){
            	var url = "${ctx}/contents/countContentsByBean.action";
        		var param = {};
        		var catalogID = $("#catalogID").val();
        		param.catalogID=catalogID;
        		$.post(url,param,function(data){
            ...
            } else if ($("#catalogType").val() == 't_accrual'){
            	 $('#accrualTypeWin').window('open');
            }else{
        ...
        $("#editContentsBtn").click(function () {
            ...
            if ($("#catalogType").val() == 't_contents' || $("#catalogType").val() == 't_specialTopic' || $("#catalogType").val() == 't_oneContent') {
            	...
            	window.location.href = "${ctx}/accrual/showAccrualEdit.action?theID=" + $("#theID_delete").val();
            }
        });
...
//         });
        ...
        contents_page.initAccrualTypeWin();
        ...
        $('#frontContentsBtn').click(function(){
        	...
        	var url = "";
        	var parentID2='';
        	if(type == 't_contents' || type == 't_oneContent'){
        		if($("#parentID").val()==''){
        			parentID2=parentID;
        		}else{
...
//                 $('#fileInfoListBtn').hide();
                $('#topNewsBtn').hide();
                $("[name='catalogID_upload']").val(pid);
            } else if (data.catalogType == 't_oneContent') {
            	$('#dg').datagrid("showColumn", "releaseDate");
                $('#dg').datagrid("showColumn", "source");
            	$('#dg').datagrid("hideColumn", "accrualTypeName");
                $('#dg').datagrid("hideColumn", "linkTypeName");
...
//                $('#dg').datagrid("hideColumn", "isindexName");
    ...
    function newAccrualPage(){
    	var type = $("#accrualType").val();
    	if(type == ""){
    		alert("请先选择权责清单类型！");
    		return;
    	}else{
    		window.location.href = "${ctx}/accrual/newAccrualPage.action?type="+type+"&catalogID="+ $("#catalogID").val();
    		contents_page.closeAccrualTypeWin();
    	}
    }
</script>
<table width="100%">
        ...
        <td valign="top" width="90%">
            ...
            <table id="dg" title="<div id='table_title'>${catalog.title}" style="width:auto;height:600px" data-options="
                ...
                <tr>
                    ...
                    <th field="accrualTypeName" width="100"><input type="hidden" field="accrualType">权责清单类型</th>
...
<div id="accrualTypeWin" class="easyui-window" title="权责清单类型选择" collapsible="false" minimizable="false"
        ...
        background: #fafafa;" data-options="left:'250px',top:'200px'">
                <div class="easyui-layout" fit="true">
                        ...
                        <form id="editCatalogForm" method="post">
                            <table cellpadding=3>
                                ...
                                <tr>
                                    <td>权责清单类型：</td>
                                    <td>
                                        <select id="accrualType">
                                            <option value="1">行政许可</option>
											<option value="2">非行政许可审批</option>
											<option value="3">行政处罚</option>
											<option value="4">行政强制</option>
											<option value="5">行政征收</option>
											<option value="6">行政给付</option>
											<option value="7">行政检查</option>
											<option value="8">行政指导</option>
											<option value="9">行政确认</option>
                    ...
                    <div region="south" border="false" style="text-align: right; height: 30px; line-height: 30px;">
                        <a class="easyui-linkbutton" icon="icon-ok" href="javascript:void(0)" onClick="newAccrualPage()">
                            确定</a> <a class="easyui-linkbutton" icon="icon-cancel" href="javascript:void(0)"
                                      onclick="contents_page.closeAccrualTypeWin()">取消</a>
                    </div>
                </div>
            </div>
</body>
