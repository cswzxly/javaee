...
public class FrontLetterAction extends BaseAction {
	...
	public String front_saveLetter(){
		logger.info(">>>>>>>>>>>>>>>>>>>>>>>FrontLetterAction:front_saveLetter>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
		Letter letter = getLetter();
		logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>acceptNum:"+letter.getAcceptNum());
		String date = DateUtils.getNowTime().replaceAll("-", "").replaceAll(":", "").replaceAll(" ", "");
		Random r = new Random();
...
//		}
	...
	public String front_queryLetterDetail(){
		...
		String strTheID = request.getParameter("theID");
		if(strTheID != null && !"".equals(strTheID)){
			Arithmetic des = new Arithmetic();
			int theID = Integer.valueOf(des.getDesString(strTheID));
	...
	public Letter getLetter(){
		...
		if(content.contains("<html>")){
			letter.setContent(StringUtils.protectAgainstXSS(request.getParameter("content")));
		}else{
