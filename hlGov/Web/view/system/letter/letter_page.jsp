...
<input type="hidden" id="_status" value=${status }>
	...
	<c:choose>
		<c:when test="${flag=='1'}">
		请选择信箱类型：<select name="flag" id="flag"><option></option><option value="1" selected="selected">县长信箱</option><option value="2">我要写信</option><option value="3">我要建议</option></select> 
		</c:when>
		<c:when test="${flag=='2'}">
		请选择信箱类型：<select name="flag" id="flag"><option></option><option value="1">县长信箱</option><option value="2" selected="selected">我要写信</option><option value="3">我要建议</option></select> 
		</c:when>
		<c:when test="${flag=='3'}">
		请选择信箱类型：<select name="flag" id="flag"><option></option><option value="1">县长信箱</option><option value="2">我要写信</option><option value="3" selected="selected">我要建议</option></select> 
		</c:when>
		<c:otherwise>
		请选择信箱类型：<select name="flag" id="flag"><option></option><option value="1">县长信箱</option><option value="2">我要写信</option><option value="3">我要建议</option></select> 
		</c:otherwise>
	</c:choose>
	
	<c:choose>
	<c:when test="${isOpen =='1'}">
		是否公开：<select name="isOpen" id="isOpen"><option></option><option value="1" selected="selected">是</option><option value="0">否</option></select> &nbsp;&nbsp;
	</c:when>
	<c:when test="${isOpen =='0'}">
		是否公开：<select name="isOpen" id="isOpen"><option></option><option value="1">是</option><option value="0" selected="selected">否</option></select> &nbsp;&nbsp;
	</c:when>
	<c:otherwise>
		是否公开：<select name="isOpen" id="isOpen"><option></option><option value="1">是</option><option value="0">否</option></select> &nbsp;&nbsp;
	</c:otherwise>
	</c:choose>
...
<table id="dg" title="列表" style="width:auto;height:640px" data-options="
    ...
    <tr>
        ...
        <th field="theme" width="180">主题</th>
        <th field="flagName" width="180"><input field="flag" type="hidden">信箱类型</th>
        <th field="status" width="180">状态</th>
        <th field="isOpen" width="180">是否公开</th>
    </tr>
    </thead>
</table>
<div id="tb" style="height:auto">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" id="letterDetailBtn">查看详情</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" id="printLetterBtn">打印详情</a>
</div>
